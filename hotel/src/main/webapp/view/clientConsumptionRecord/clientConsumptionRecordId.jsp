    ...
    <script>
        $(document).ready(function () {
            ...
            $(".openDeleteReservation").click(function(){
            	...
            	$("#okDeleteReservation").attr('href',"/deleteClientrConsumptionRecord?clientConsumptionRecordId="+v_id);
            })
            ...
            <div class="main-right  col-md-10 col-md-offset-2 bodys">
                        <div class="panel-body navs">
                            <ol class="breadcrumb">
                                ...
                                <li><a href="${pageContext.request.contextPath}/showAllClientConsumptionRecord/1">普通客户消费记录</a></li>
                            	<c:set var="a" value="1"/>
                                <c:if test="${requestScope.clientConsumptionRecords.size() >0}">
                                <c:if test="${a == 1}">
                                <c:forEach items="${requestScope.clientConsumptionRecords}" var="clientConsumptionRecords">
                                <li><a href="#">${clientConsumptionRecords.clientName}的消费记录</a></li>
                                </c:forEach>
                                <c:set var="a" value="2"/>
                                </c:if>
                                </c:if>
          	...
          	<div class="col-md-12">
                    <div class="panel panel-default tableTitle">
                        <div class="panel-heading">
                          	搜索客户：
                          	<form action="${pageContext.request.contextPath}/showInputlientConsumptionRecord/1" method="post">  
                        ...
                        <div class="panel-body">
                            <table class="table table-bordered tb-hover">
                                ...
                                <tbody>
                                	...
                                	<c:forEach items="${requestScope.clientConsumptionRecords}" var="clientConsumptionRecords">
                                    <tr class="users">                                                                
                                        <td class="userRealName">${clientConsumptionRecords.clientName}</td>
                                        <td class="userVipId">${clientConsumptionRecords.clientIdCard}</td>
                                        <td class="userIdCard">${clientConsumptionRecords.clientPhone}</td>                                        
                                        <td class="userPhone">${clientConsumptionRecords.clientConsumptionTime}</td>
                                        <td class="userPhone">${clientConsumptionRecords.clientConsumptionMoney}</td>
                                        <c:if test="${clientConsumptionRecords.clientConsumptionManner == 0}">
                                        	<td class="userGender">现金支付</td>
                                        </c:if>
                                        <c:if test="${clientConsumptionRecords.clientConsumptionManner == 1}">
                                        	<td class="userGender">扫码支付</td>
                                        </c:if>
                                        <c:if test="${clientConsumptionRecords.clientConsumptionManner == 2}">
                                        	<td class="userGender">刷卡支付</td>
                                        </c:if>
                                        <td class="userPhone">${clientConsumptionRecords.staffName}</td>
                                        <td class=" text-center">                                                                        
                                        &nbsp;&nbsp;<i title="删除记录"  id="${clientConsumptionRecords.clientConsumptionRecordId}" class="glyphicon glyphicon-remove openDeleteReservation"></i></td>
                                    </tr>
                                    </c:forEach>
                                    </c:if>
                                </tbody>
                        ...
                        <div class="panel-body spligs">
                            <nav>
                                <ul class="pagination">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/showClientConsumptionRecord/${requestScope.nowPage -1}&${requestScope.clientId}" aria-label="Previous">
							    ...
							    <li>				    		                       
							    <a style="background:#e7e7e7" href="/showClientConsumptionRecord/${showPage}&${requestScope.clientId}">${showPage}</a>				    			    
							    </li>
							    </c:if>	
							     <c:if test="${requestScope.nowPage != showPage}">	 
							    <li>				    		                       
							    <a href="/showClientConsumptionRecord/${showPage}&${requestScope.clientId}">${showPage}</a>				    			    
							    </li>
							    </c:if>	
							    </c:forEach>
                                    <li>
                                        <a href="${pageContext.request.contextPath}/showClientConsumptionRecord/${requestScope.nowPage +1}&${requestScope.clientId}" aria-label="Next">
